...
No model metrics are hard-coded or estimated in this README. Real validation and final test results are generated by running:

```bash
python train_model.py
```

The training script prints the Random Forest and XGBoost validation comparison, selects the best algorithm, evaluates the fresh final model once on the untouched test set, and saves the actual metrics in `models/model_info.json`.
...
After the project runs successfully locally, keep deployment package versions compatible with the versions used to save the trained model and scaler. Avoid adding untested version pins.
